...
module tb_abp_top;
  ...
  initial begin
    $dumpfile("tb_abp_top.vcd");
    $dumpvars(0, tb_abp_top);
  end
  ...
  task test_read;
    ...
    begin
      ...
      #10;  // Setup phase
      #10;  // Access phase
      $display("Read Operation Completed: Address = %h, Data Read = %h", address,
               apb_read_data_out);
      transfer = 0;  // Deassert transfer signal
      read = 0;
      #10;
  ...
  task test_address_decoding;
    begin
      ...
      test_write(9'h005, 8'hA5);  // Target Slave 2
      test_write(9'h085, 8'h5A);  // Target Slave 1
    end
  endtask
  ...
  task test_read_with_wait_states;
    ...
    begin
      ...
      transfer = 0;
      read = 0;
      $display("Read Completed with Wait States: Address = %h, Data Read = %h", address,
               apb_read_data_out);
      #10;
    end
  endtask
  ...
  task test_out_of_range_address;
    ...
    begin
      ...
      if (pslverr)
        ...
        $display(
            "No error detected for invalid address %h, PSLVERR = %b", invalid_address, pslverr
        );
      #10;
    end
  endtask
